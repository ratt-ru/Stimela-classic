...
def compute_vis_noise(msname, sefd, spw_id=0):
    ...
    print(
        ">>> SEFD of %.2f Jy gives per-visibility noise of %.2f mJy"
        % (sefd, noise * 1000)
    )
...
if addnoise:
    noise = params.pop("noise", 0) or compute_vis_noise(msname, params.pop("sefd", 551))
...
if beam and beam_files_pattern:
    beam_opts = {
        ...
        if params.pop("parallactic-angle-rotation", False)
        else 0,
        "me.e_all_stations": 1,
...
if exitcode != 0:
    raise RuntimeError(
        f"Meqtrees failed with exit code {exitcode}. See logs for more details"
    )
