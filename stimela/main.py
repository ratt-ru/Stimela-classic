...
def build(argv):
    ...
    parser.add_argument("-b", "--base", action="store_true", help="Build base images")
    ...
    parser.add_argument(
        "-uo", "--us-only", help="Only build these cabs. Comma separated cab names"
    )
    ...
    parser.add_argument(
        "-p", "--podman", action="store_true", help="Build images using podman."
    )
    ...
    if args.cab:
        raise SystemExit(
            "DEPRECATION NOTICE: This feature has been deprecated. Please specify your \
    ...
    if args.base:
        ...
        return 0
    raise SystemExit(
        "DEPRECATION NOTICE: The building of cab images has been deprecated"
    )
...
def run(argv):
    ...
    parser = ArgumentParser(
        description="Dockerized Radio Interferometric Scripting Framework.\n"
    ...
    add("-in", "--input", help="Input folder")

    add("-out", "--output", help="Output folder")
    ...
    add("-pf", "--pull-folder", help="Folder to store singularity images.")

    add("script", help="Run script")
    ...
    add(
        "-jt",
        "--job-type",
        choices=["docker", "singularity", "podman"],
    ...
    add(
        ...
        "--log-level",
        default="INFO",
        choices=loglevels.upper().split() + loglevels.split(),
    ...
    _globals = dict(
        _STIMELA_INPUT=args.input,
        _STIMELA_OUTPUT=args.output,
...
def pull(argv):
    ...
    add("-at", "--all-tags", action="store_true", help="Pull all tags for this image")
    ...
    for _cab in args.cab_base or []:
        if _cab in CAB:
            ...
            for tag in tags:
                ...
                repository_.append(
                    param["hub"] if "hub" in param.keys() else args.repository
                )

    repository_ = repository_ + (
        [args.repository] * len(args.image) if args.image is not None else []
    )
    ...
    if args.image:
        for hub, image in zip(repository_, args.image):
            ...
            if args.singularity:
                ...
                if args.podman:
                    podman.pull(
                        "/".join([hub, image]) if hub != "" else image, force=args.force
                    )
                else:
                    docker.pull(
                        "/".join([hub, image]) if hub != "" else image, force=args.force
...
def main(argv):
    ...
    parser = ArgumentParser(
        description="Stimela: Dockerized Radio Interferometric Scripting Framework. "
        "|n version {:s} |n install path {:s} |n "
        "Sphesihle Makhathini <sphemakh@gmail.com>".format(
            stimela.__version__, os.path.dirname(__file__)
        ),
    ...
    add("-h", "--help", action="store_true", help="Print help message and exit")
...
""")
    ...
    try:
        ...
        raise KeyError(
            "Command '{:s}' not recognized Run : 'stimela help' for help".format(cmd)
        )
