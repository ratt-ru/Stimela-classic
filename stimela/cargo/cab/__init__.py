...
from stimela.exceptions import StimelaCabParameterError
import time
    ...
    def validate(self, value):
        if self.choices and value not in self.choices:
            raise StimelaCabParameterError(
                "Parameter '{0}', can only be either of {1}".format(
        ...
        for item in self.dtype:
            if isinstance(item, tuple):
                ...
                if isinstance(value, t):
                    ...
                    types = (
                        (t, int) if t is float else (t,)
                    )  # float permits ints as well
                    if all(
                        isinstance(x, types) for x in value
                    ):  # check that all elements are of permitted type
                        return True
            elif item == "file":
                return True
                ...
                return True
        raise StimelaCabParameterError(
            "Expecting any of types {0} for parameter '{1}', but got '{2}'".format(
        ...
        def _type(a):
            if a == "file":
                if self.io not in ["input", "output", "msfile"]:
                    raise StimelaCabParameterError(
                        "io '{0}' for parameter '{1}' not understood. Please specify 'io' as either 'input', 'output' or 'msfile'".format(
                            self.io, self.name
                        )
                    )
                return "file"
...
class CabDefinition(object):
        ...
        if parameter_file:
            ...
            for param in parameters0:
                ...
                addme = Parameter(
                    ...
                    info=param.get("info", None)
                    or "No documentation. Bad! Very bad...",
                    default=default,
    ...
    def display(self, header=False):
        ...
        for param in self.parameters:
            ...
            print(
                "  Name         {}{}".format(
                    param.name, "/{}".format(param.mapping) if param.mapping else ""
                )
            )
    ...
    def toDict(self):
        ...
        for param in self.parameters:
            if isinstance(param.dtype[0], tuple):
                if (
                    not isinstance(param.value, (list, tuple))
    ...
    def update(self, options, saveconf, tag=None):
        ...
        for param0 in required:
            if (
                param0.name not in options.keys()
                and param0.mapping not in options.keys()
            ):
                raise StimelaCabParameterError(
                    "Parameter {} is required but has not been specified".format(
                        param0.name
                    )
                )
        ...
        for name, value in options.items():
            ...
            for param in self.parameters:
                if name in [param.name, param.mapping]:
                    ...
                    if param.deprecated:
                        self.log.warning(
                            f"Parameter {name} for cab {self.task} is deprecated, and will be removed in a future release"
                        )
                    if param.io:
                        if value is None:
                            continue
                        ...
                        for _value in value:
                            if isinstance(_value, pathformatter):
                                if param.check_io:
                                    raise StimelaCabParameterError(
                                        "Pathformatters cannot be used on io parameters where io has to be checked"
                                    )
                                ...
                                for p in joinlist:
                                    if not isinstance(p, placeholder):
                                        ...
                                        if p() not in IODEST.keys():
                                            raise StimelaCabParameterError(
                                                "The location '{0}' specified for parameter '{1}', is unknown. Choices are {2}".format(
                                ...
                                if len(val) == 2:
                                    ...
                                    self.log.info(
                                        "Location of '{0}' was specified as '{1}'. Will overide default.".format(
                                            param.name, val[1]
                                        )
                                    )
                                    _value = val[0]
                                ...
                                if location in ["input", "msfile"]:
                                    ...
                                    if param.check_io and not os.path.exists(path):
                                        raise StimelaCabParameterError(
                                            "File '{0}' for parameter '{1}' could not be located at '{2}'.".format(
                                ...
                                raise StimelaCabParameterError(
                                    "io parameter must either be a pathformatter object or a string"
                                )
                        ...
                        if isinstance(value, pathformatter):
                            raise StimelaCabParameterError(
                                "Path formatter type specified, but {} is not io".format(
                                    param.name
                                )
                            )

                        self.log.debug("Validating parameter {}".format(param.name))
                        param.validate(value)
                        param.value = value
            if not found:
                raise StimelaCabParameterError(
                    "Parameter {0} is unknown. Run 'stimela cabs -i {1}' to get help on this cab".format(
                        name, self.task
                    )
                )
        conf = {}
