...
def build(image, build_path, tag=None, build_args=None, fromline=None, args=[]):
    ...
    if build_args:
        ...
        stdw.flush()
        utils.xrun(
            "docker build", args + ["--force-rm", "-f", stdw.name, "-t", image, bdir]
        )

        stdw.close()
    else:
        utils.xrun("docker build", args + ["--force-rm", "-t", image, bdir])
...
class Container(object):
    ...
    def add_volume(self, host, container, perm="rw", noverify=False):
        if os.path.exists(host) or noverify:
            ...
            raise IOError(
                "Directory {0} cannot be mounted on container: File doesn't exist".format(
                    host
                )
            )
    ...
    def create(self, *args):
        ...
        self._print(
            "Instantiating container [{}]. The container ID is printed below.".format(
                self.name
            )
        )
        utils.xrun(
    ...
    def info(self):
        output = subprocess.check_output(
            "docker inspect {}".format(self.name), shell=True
        ).decode()
    ...
    def get_log(self):
        ...
        exit_status = subprocess.call(
            "docker logs {0}".format(self.name),
            stdout=stdout,
            stderr=stdout,
            shell=True,
        )
        if exit_status != 0:
            self.logger.warn(
                "Could not log container: {}. Something went wrong durring execution".format(
                    self.name
                )
            )
    ...
    def stop(self):
        ...
        if status in ["running", "paused"]:
            try:
                ...
                "Received terminate signal. Will stop and remove container first"
            ):
                killed = True
        self.status = "exited"
