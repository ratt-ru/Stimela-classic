...
def build(image, build_path, tag=None, build_args=None, fromline=None, args=[]):
    ...
    if build_args:
        ...
        utils.xrun(
            "podman build",
            args + ["--force-rm", "--no-cache", "-f", stdw.name, "-t", image, bdir],
        )

        stdw.close()
    else:
        utils.xrun(
            "podman build", args + ["--force-rm", "--no-cache", "-t", image, bdir]
        )
...
class Container(object):
    ...
    def info(self):
        output = subprocess.check_output(
            "podman inspect {}".format(self.name), shell=True
        ).decode()
    ...
    def get_log(self):
        ...
        exit_status = subprocess.call(
            "podman logs {0}".format(self.name),
            stdout=stdout,
            stderr=stdout,
            shell=True,
        )
