    ...
    def poll(self, timeout=5, verbose=False):
        while True:
            try:
                ...
                if verbose:
                    self.log.debug(
                        "poll() exception: {}".format(traceback.format_exc())
                    )
...
def xrun_nolog(command, name=None):
    ...
    if status:
        raise StimelaProcessRuntimeError(
            "{} returns error code {}".format(name, status)
        )
...
def xrun(
    ...
    try:
            ...
            for fname, fobj in fdlist:
                ...
                if line is not None:
                    log.log(
                        ...
                        line,
                        extra=dict(
                            stimela_subprocess_output=(command_name, stream_name)
                        ),
                    )
        ...
        raise StimelaCabRuntimeError(
            "{}: SystemExit with code {}".format(command_name, status)
        )

    except KeyboardInterrupt:
        if callable(kill_callback):
            log.warning(
                "Ctrl+C caught: shutting down {} process, please give it a few moments".format(
                    command_name
                )
            )
            kill_callback()
            log.info(
                "the {} process was shut down successfully".format(command_name),
        ...
        proc.wait()
        raise StimelaCabRuntimeError(
            "{} throws exception '{}'".format(command_name, str(exc))
        )

    if status:
        raise StimelaCabRuntimeError(
            "{} returns error code {}".format(command_name, status)
        )
