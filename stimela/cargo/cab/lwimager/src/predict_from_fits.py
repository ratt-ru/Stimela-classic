...
if naxis<2:
    ...
    sys.stdout.write('WARNING::FITS image has 2 axes. Will add STOKES and FREQ axes\n')
elif naxis == 3:
    ...
    if third_axis is None:
        sys.stdout.write('WARNING:: Third axis is not labelled, assuming its the FREQ Axis\n')
        ...
        sys.stdout.write('WARNING:: CTYPE3 value [{}] is unknown, will ignore it and assume 3rd axis is FREQ\n'.format(third_axis))
        hdu.data = hdu.data[:,numpy.newaxis,...]

elif naxis == 4:
    sys.stdout.write('FITS image has four axes. If the axes are labeled (via CTYPE<axis index>), will attempt to restructure it so it is fit to predict from\n')
    third_axis = hdu.header.get('CTYPE3', None)
    if third_axis is None:
        raise RuntimeError('ABORT:: FITS axes are not labelled. Cannot proceed. Please label STOKES and FREQ accordingly\n')
    elif third_axis.lower()=='freq':
        hdu.data = numpy.rollaxis(hdu.data, 1)
else:
    sys.stdout.write('WARNING:: FITS image has more than 4 axes. Passing the  buck to lwimger\n')
