...
class Container(object):
    def __init__(self, image, name,
        ...
        self.cont_logger = utils.logger.StimelaLogger(
            log_container or stimela.LOG_FILE, jtype="udocker")
    ...
    def info(self):
        ...
        output = subprocess.check_output(
            "udocker inspect {}".format(self.name), shell=True).decode()
    ...
    def create(self):
        ...
        self._print("Creating container [{0:s}]. Timeout set to {1:d}. The container ID is printed below.".format(
            self.name, self.time_out))
        utils.xrun(
            "udocker", ["create", "--name={0:s}".format(self.name), self.image])
