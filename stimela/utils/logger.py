...
class MultiplexingHandler(logging.Handler):
    ...
    def emit(self, record):
        handler = (
            self.err_handler
            if record.levelno > logging.INFO and self.multiplex
            else self.info_handler
        )
        handler.emit(record)
...
class ConsoleColors:
    ...
    def colorize(msg, *styles):
        style = "".join(styles)
        return msg.replace(ConsoleColors.BEGIN, style).replace(
            ConsoleColors.END, ConsoleColors.ENDC if style else ""
        )
...
class ColorizingFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        return ConsoleColors.colorize(
            super(ColorizingFormatter, self).format(record),
            style or self._default_color,
        )
