...
if not padding:
    filter(lambda x: x['name'] == 'size', params)[
        0]["value"] = trim  # npix is now the unpadded size
    # replace with 'padding' argument
    params.append({'name': 'padding', 'value': pad})
...
if '{0}auto-threshold'.format(cab['prefix']) not in args:
    ...
    for item1 in args:
        if 'noise-image' in item1:
            noise_image = item1.split(
                '{0}noise-image '.format(cab['prefix']))[-1]
            args.remove(
                '{0}noise-image {1}'.format(cab['prefix'], noise_image))
                ...
                if 'noise-sigma' in item2:
                    ...
                    for item3 in args:
                        if '{0}threshold'.format(cab['prefix']) in item3:
                            args.remove(item3)
                    args.append('{0}threshold {1}'.format(
                        cab['prefix'], threshold))
    if not removed:
